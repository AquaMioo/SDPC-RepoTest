...
## Every OTP purpose needs its own subject and reason in EmailOneTimePassword
EmailOneTimePassword::subject() and reason() are match expressions with no default. Notification::fake() never builds the email, so a OneTimePasswordPurpose case missing from them passes every feature test and throws only when a code is really mailed. SchoolEmail shipped that way (fixed 2026-09-20). When adding a purpose, add both lines; RegistrationOtpTest::test_every_purpose_renders_an_email renders every case.
