...
## One OTP mechanism, one purpose per door
App\Services\Verification\OneTimePasswordService backs every code: Registration, Appeal, SchoolEmail, Login and SetPassword (OneTimePasswordPurpose). The purpose is part of the unique key, so a code minted for one door cannot be replayed at another — add a new case rather than reusing one. Codes are hashed at rest, expire, carry an attempt budget, and have a resend floor — see config/otp.php. A correct code is consumed on the way out, so pressing back does not open the door twice.
