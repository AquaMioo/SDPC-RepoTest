...
## Group chats are invite-only — except the thread for the build the team is on
conversations.student_team_id only says which team a thread belongs to. It lets nobody read the thread. Access is the thread's student, plus conversation_members rows (checked together with live membership of that team), plus the client's team. Only the team's creator (ownsTeam) may add or remove rows, through ConversationMemberController, and never the thread's student or themselves. adoptStudentTeam() attaches a team only to threads owned by its creator, so a member's own client threads stay private.

The one exception, asked for by the testers on 2026-09-23: the thread for the project the team is actually building. App\Actions\Messaging\SeatTeammatesInProjectChat writes a conversation_members row for every teammate of the student who holds the build, and attaches the team to the thread if it carries none. It runs when a student joins a team that is already building (JoinTeam) and when a client takes the lead on (RespondToApplication, RespondToInvitation); migration 2026_09_23_090500 wrote the rows for teams that were already building. A teammate is tied to that project and answers for its deadlines, so leaving them out of the conversation with the client left them working blind — and yes, that means a latecomer reads what was said before they joined.

Every other thread stays the creator's to share: a business that invited them to a posting they never took is not the team's work. Do not widen this to "every team member shares every thread".
...
## Removing a message is two different things, and only one of them broadcasts
`remove()` writes `messages.removed_at`: the sender takes a line back from everybody, the row stays and says "Message removed", and it broadcasts. `hide()` writes a `message_hides` row: one viewer stops being sent one line, nothing changes for anybody else, so it must NOT broadcast and is open to either side of the thread on anyone's message.
